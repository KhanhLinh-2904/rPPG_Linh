...
class GUI(QMainWindow, QThread, QApplication):
    ...
    def main_loop(self):
        ...
        if color_frame is not None:
            ...
            QApplication.processEvents()
            (bpm, _, _)  = self.runAllModels.run(color_frame)  # Run the main algorithm
            # print("bpm: ",bpm)
            self.update_bpm_and_hrv_and_fre(bpm)
           
            if len(self.runAllModels.bpms) > 25:
                # print("===========: ",len(self.runAllModels.bpms))
                # print("self.runAllModels.bpms: ",self.runAllModels.bpms)
                
                for i in range(5, 0, -1):
                    try:
                        if(len(self.runAllModels.bpms[-5 * i:-5 * (i - 1)])==0):
                            continue
                        self.smooth_bpms.append(np.mean(self.runAllModels.bpms[-5 * i:-5 * (i - 1)]))
    ...
    def run(self):
        ...
        while self.running:
            self.main_loop()
            self.signal_Plt.clear()
            self.signal_Plt.plot(self.runAllModels.RGB_signal_buffer, pen='r')  # Plot green signal
